...
test.describe('Phoenix System Integration Tests', () => {
  ...
  // Helper function to create admin headers for privileged endpoints
  ...
  test.describe('Complete Phoenix Workflow', () => {
    ...
    test('should perform complete setup workflow via API', async ({ request }) => {
      ...
      // Step 3: Get system metrics (requires admin auth)
  ...
  test.describe('Error Recovery and Resilience', () => {
    ...
    test('should validate system state after operations', async ({ request }) => {
      ...
      // Validate through metrics API (requires admin auth)
      ...
      expect(metrics.data.overview.totalWorlds).toBeGreaterThanOrEqual(3)
  ...
  test.describe('Backup and Recovery Integration', () => {
    ...
    test('should validate backup data integrity via metrics', async ({ request }) => {
      ...
      // Get metrics to validate backup integrity (requires admin auth)
      const metricsResponse = await request.get('/api/phoenix/metrics?environment=LOCAL', {
        headers: createAdminHeaders()
      })
      expect(metricsResponse.ok()).toBeTruthy()
      ...
      expect(metrics.success).toBe(true)
      expect(metrics.data.overview).toHaveProperty('totalWorlds')
      expect(metrics.data.usage).toHaveProperty('worldUsage')
      
      // Since we created a test world, there should be some worlds
      if (metrics.data.overview.totalWorlds > 0) {
        expect(metrics.data.usage.worldUsage).toEqual(expect.any(Array))
      }
