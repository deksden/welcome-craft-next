# ⚙️ WelcomeCraft: Операционные Процессы (Workflows)
...
Процесс используется при получении от пользователя сведений об ошибке в системе. ОБНОВЛЕН для интеграции с трехуровневой
системой тестирования. Шаги процесса:
...
4. **✍️ Написание спецификации бага:** **Я выполняю процесс `WF-05`** для создания исполняемой спецификации в
...
### ✅ WF-02: Фикс без теста

Процесс применяется, когда обнаруженный баг не предполагает создания теста для воспроизводства: он или незначителен, или
не затрагивает существенный функционал и не ожидается в регрессии, или это ошибка компиляции/lint, или еще какая то
техническая проблема. Это упрощенная версия процесса "WF-01 Работа с ошибкой", только без теста и верификация
выполняется пользователем. Процесс идет по таким шагам:
...
4. **✅ Верификация фикса:**
    ...
    * если обнаружил неподтвержденные пользователем фиксы, напоминай пользователю про необходимость верифицировать
