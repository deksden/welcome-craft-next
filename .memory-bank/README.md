...
2. **Отсутствие дублирования (DRY):** Информация не повторяется, используются ссылки
...
- **WF-02:** Фикс без теста (для незначительных проблем)
- **WF-03:** Обновление Memory Bank (фиксация опыта)
- **WF-04:** Архивирование (регулярная очистка, поддержание актуальности)
...
* Убеждаюсь, что аутентификация будет использовать `universalAuthentication`.

6) 🔴 Написание провального теста (RED):

* Создаю тест в `tests/e2e/regression/` (например, с использованием `createUseCaseTest()` helper, если он адаптирован под `universalAuthentication`, или напрямую).
* **НОВОЕ:** Использую world isolation, `universalAuthentication` и AI fixtures для стабильного воспроизведения.
...
* Обновляю `.memory-bank/testing/testing-overview.md` и `.memory-bank/testing/api-auth-setup.md`, если были изменения в подходах к тестированию или аутентификации.
```
...
#### Периодичность:
- **Регулярно:** Каждые 2-3 недели
- **По потребности:** При накоплении >5 завершенных задач  
- **Перед крупными рефакторингами:** Очистка для новых циклов
- **После major milestones:** E2E унификация, архитектурные изменения

#### Последние архивирования:
- **2025-06-28:** E2E унификация и документация (archive-2025-06-28)
- **2025-06-21:** UC-10 CMS и testing excellence (archive-2025-06-21)

#### Инструменты:
- **Команда:** `/mbank "Архивируй завершенные задачи"`
- **Ручная проверка:** Просмотр папки `tasks/` на предмет готовых к архивированию материалов
...
> **Memory Bank — единственная связь с предыдущей работой. Его точность определяет эффективность каждой новой сессии.**
...
* Создаю тест в `tests/e2e/use-cases/` (например, с использованием `createUseCaseTest()` helper, если он адаптирован, или напрямую).
* **НОВОЕ:** Использую world isolation, `universalAuthentication` и AI fixtures для детерминистичности.
