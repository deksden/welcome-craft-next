...
**Версия:** 6.1.0  
**Дата:** 2025-07-03  
**Статус:** E2E PROJECT NAMING - Переименованы E2E проекты в e2e-core и e2e-admin для четкого разделения функциональности
...
- **AI Testing:** AI Fixtures Provider v2.0.0 (lossless архитектура)
...
```bash
pnpm test             # Все тесты (unit + api + e2e) с эфемерной БД
pnpm test:unit        # Юнит-тесты (219+ тестов)
pnpm test:api         # API/интеграционные тесты (119+ тестов)
pnpm test:e2e         # End-to-End тесты (e2e-core + e2e-admin)

# 📋 E2E проекты (логика именования):
# - e2e-core: Use Cases + Components + Regression
# - e2e-admin: Phoenix административная система
playwright test --project=e2e-core   # Только основная функциональность
playwright test --project=e2e-admin  # Только Phoenix admin тесты
