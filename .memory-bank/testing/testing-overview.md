...
### Четкое разделение инструментов тестирования

#### 🧪 **Vitest** — Только юнит-тесты с моками
- **Назначение:** Тестирование изолированной бизнес-логики, утилит, хелперов
- **Подход:** Полное мокирование всех внешних зависимостей
- **Что тестируем:**
  - Чистые функции и утилиты
  - Бизнес-логика без побочных эффектов
  - Валидация и трансформация данных
  - Парсеры, форматеры, вычисления
- **Мокируем:** База данных, API, `server-only` модули, файловую систему
- **Скорость:** Максимально быстрые тесты (< 1s на весь набор)

#### 🎭 **Playwright** — Интеграционные и E2E тесты
- **Назначение:** Тестирование API endpoints и полных пользовательских сценариев
- **Подход:** Реальные запросы к приложению, минимум моков
- **Что тестируем:**
  - **Route Tests:** API endpoints с реальной БД и аутентификацией
  - **E2E Tests:** Полные пользовательские workflows через браузер
  - **Integration Tests:** Взаимодействие компонентов системы
- **Реальная инфраструктура:** Ephemeral PostgreSQL БД, real HTTP requests
- **Производительность:** 30-60s на полный набор тестов

### Пирамида тестирования WelcomeCraft

```
       /\     E2E Tests (Playwright)      /\
      /  \    Пользовательские сценарии  /  \  ← Медленные, но максимально реалистичные
     /____\                             /____\
    /      \   Route Tests (Playwright) /      \
   /        \  API + Integration       /        \ ← Средняя скорость, реальная БД
  /__________\                        /__________\
 /            \ Unit Tests (Vitest)  /            \
/              \ Бизнес-логика      /              \ ← Быстрые, максимально изолированные
```

**Принцип разделения:** 
- **Если тестируешь логику БЕЗ внешних зависимостей** → Vitest + моки
- **Если тестируешь API endpoints или UI workflows** → Playwright + реальная инфраструктура
...
### ✅ Playwright — Интеграционное и E2E тестирование

**Архитектурные принципы:**
- **Production Build Testing:** Все тесты выполняются против `pnpm build && pnpm start` для устранения dev-server нестабильности
- **Real Infrastructure:** Ephemeral PostgreSQL БД (порт 5433), реальные HTTP запросы
- **Multi-domain Support:** `app.localhost` vs `localhost` архитектура
- **Universal Authentication:** Единая система аутентификации через `test-session` cookies

**Route Tests примеры:**
```typescript
// ✅ API endpoint тестирование с реальной БД
test('should create artifact via API', async ({ request }) => {
  const response = await request.post('/api/artifact', {
    data: { kind: 'text', title: 'Test', content: 'Hello' }
  })
  expect(response.status()).toBe(200)
})

// ✅ Phoenix admin endpoints
...
**E2E Tests примеры:**
```typescript
// ✅ Полные пользовательские workflows
test('User can publish site through UI', async ({ page }) => {
  await universalAuthentication(page, testUser)
  const siteEditor = new SiteEditorPage(page)
  await siteEditor.createAndPublishSite('My Site')
  await expect(page.getByText('Site published')).toBeVisible()
})
```

### ✅ Vitest — Юнит-тестирование с полным мокированием

**Архитектурные принципы:**
- **Full Isolation:** Полное мокирование всех внешних зависимостей
- **Fast Execution:** Максимальная скорость выполнения (< 1s на весь набор)
- **Pure Logic Testing:** Фокус на бизнес-логике без побочных эффектов
- **No Real Dependencies:** Никаких реальных БД, HTTP запросов, файловой системы
...
describe('artifactValidation', () => {
  it('should validate artifact title length', () => {
    const result = validateArtifactTitle('Valid title')
    expect(result.isValid).toBe(true)
  })
})

// ✅ Мокирование server-only модулей
vi.mock('server-only')
...
**❌ Vitest анти-паттерны:**
```typescript
// ❌ НЕ используй реальную БД в Vitest
import { db } from '@/lib/db' // Должно быть замокано!

// ❌ НЕ делай HTTP запросы в юнит-тестах
await fetch('/api/endpoint') // Используй Playwright для этого!

// ❌ НЕ тестируй server-only компоненты в Vitest
import { ServerComponent } from '@/app/page' // Server Component!
```

**❌ Playwright анти-паттерны:**
```typescript
// ❌ НЕ мокируй БД в Playwright routes тестах
vi.mock('@/lib/db') // Используй реальную ephemeral БД!

// ❌ НЕ импортируй server-only в E2E тестах
import { getServerSession } from 'next-auth' // Server-only!

// ❌ НЕ используй Direct Function Calls в routes тестах
const result = await POST(request) // Используй HTTP request!
```
...
### Команды тестирования с разделением инструментов

```bash
# 🧪 VITEST — Юнит-тесты с моками (быстрые)
pnpm test:unit         # 269/269 ✅ — Бизнес-логика, утилиты, валидация
pnpm test:unit:watch   # Watch mode для разработки

# 🎭 PLAYWRIGHT — Интеграционные тесты (с реальной инфраструктурой)
pnpm test:routes       # 109/109 ✅ — API endpoints с ephemeral БД
pnpm test:e2e          # 40/40 ✅ — E2E пользовательские сценарии
pnpm test:ui           # Playwright UI mode для интерактивной отладки

# 🔄 COMBINED — Все тесты вместе
pnpm test              # Vitest + Playwright (все типы тестов)

# 🤖 AI Fixtures управление (только для E2E)
AI_FIXTURE_MODE=record pnpm test:e2e    # Запись новых AI фикстур
AI_FIXTURE_MODE=replay pnpm test:e2e    # Воспроизведение (по умолчанию)
```

**Принцип выбора команды:**
- **Разрабатываешь логику?** → `pnpm test:unit:watch`
- **Тестируешь API?** → `pnpm test:routes`  
- **Проверяешь UI workflow?** → `pnpm test:e2e`
- **Полная проверка перед коммитом?** → `pnpm test`
